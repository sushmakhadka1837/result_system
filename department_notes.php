...
<head>
    ...
    <style>
        .subject-card { padding:15px;border-radius:8px;box-shadow:0 2px 6px rgba(0,0,0,0.1); margin-bottom:10px; transition:0.3s;}
        .subject-card:hover { transform:translateY(-3px); box-shadow:0 5px 15px rgba(0,0,0,0.2);}
    </style>
</head>
<body>
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-3">
            ...
            <select id="semester_id" class="form-control">
                ...
                <?php for($s=1;$s<=8;$s++): ?>
                    <option value="<?= $s ?>" <?= $sem_id==$s?'selected':'' ?>><?= $s ?> Semester</option>
                <?php endfor; ?>
            </select>
        </div>
    </div>
...
$(document).ready(function(){
    ...
    $('#batch_id,#semester_id').change(function(){
        ...
        if(sem){
            window.location.href = `department_notes.php?dept_id=<?= $dept_id ?>&batch=${batch}&sem_id=${sem}`;
        }
    });
});
</script>
...
?>
    <div class="col-md-4">
        <a href="view_notes.php?subject_id=<?= $sub['id'] ?>" style="text-decoration:none;color:inherit;">
            <div class="subject-card">
                <h5><?= $sub['subject_name']; ?></h5>
                <p class="text-muted"><?= $sub['subject_code']; ?></p>
            </div>
        </a>
    </div>
<?php
        endwhile;
    }else echo "<p class='text-muted'>No subjects available for this batch & semester.</p>";
}else echo "<p class='text-muted'>Please select batch and semester.</p>";
?>
</div>
</div>
 <button
